...
def get_table_析(page, strs):
    ...
    list_析1 = page.query_selector_all('//*[@id="zhanji_01"]/div[3]/div/p')
...
################################处理%比数据#######################################
###################################-----start run_ou--------#########################
def run_ou(playwright: Playwright,qihao,url_order,game_date,zhudui_name,kedui_name,zhudui_paiming,kedui_paiming,zhudui_scored_all,kedui_scored_all,url):
    ...
    # selectors_欧 = '//*[@id="table_cont"]/table/tbody/tr'
    selectors_欧 = '#datatb > tbody>tr'
    ######获取对阵htmls
    strs_duizheng='//*[@id="odds_nav_cz"]/li[2]/div/div[2]/table/tbody'
    # html_duizhen=get_table_duizhen(page,strs_duizheng)
    selectors_必发 ='//div[8]/div[1]/table/tbody/tr'
...
###################################-----start run_ya--------#########################
def run_ya(playwright: Playwright,url,qihao):
    browser = playwright.chromium.launch(headless=True)
    context = browser.new_context()
    page = context.new_page()
    js = """
...
def run_fenxi(playwright
     : Playwright, url, qihao, url_order, game_date, zhudui_name, kedui_name, zhudui_paiming, kedui_paiming, zhudui_scored_all, kedui_scored_all):
    browser = playwright.chromium.launch(headless=True)
    context = browser.new_context()
    page = context.new_page()
    js = """
    ...
    print('li_obj_list_析1:',li_obj_list_析1)
    print('li_obj_list_析2:', li_obj_list_析2)
...
#     df_all = pd.read_excel(r'.\htmls_数据_all_dq.xlsx', parse_dates=['game_date'], date_format='%Y/%m/%d')
#
...
#
#         print(f"qihao={qihao}")
#         data_欧= run_ou(playwright, qihao,url_order,game_date,zhudui_name,kedui_name,zhudui_paiming,kedui_paiming,zhudui_scored_all,kedui_scored_all,ouzhi_urls)
#         data_欧_all = pd.concat([data_欧_all, data_欧])
#
#
#         ###获取亚赔并整理数据
#         data_亚= run_ya(playwright, yazhi_urls, qihao)
#         data_亚_all = pd.concat([data_亚_all, data_亚])
#         ###获取数据分析并整理数据----对阵近10场胜负，进失球数
#         data_析1,data_析2 = run_fenxi(playwright, shuju_urls, qihao, url_order, game_date, zhudui_name, kedui_name, zhudui_paiming, kedui_paiming, zhudui_scored_all, kedui_scored_all)
...
#         data_必发 = run_bf(playwright, touzhu_urls, qihao, url_order, shaishi, game_date)
#
...
def save_data(data_欧_all, data_亚_all, data_析1_all, data_析2_all, data_必发_all, file_suffix):
    data_欧_all.reset_index(drop=True, inplace=True)
    data_亚_all.reset_index(drop=True, inplace=True)
    data_析1_all.reset_index(drop=True, inplace=True)
    data_析2_all.reset_index(drop=True, inplace=True)
    ...
    data_all = pd.concat([data_欧_all, data_亚_all, data_析1_all, data_析2_all, data_必发_all], axis=1)
    data_all.to_excel(f'data_当前_all数据_补_{file_suffix}.xlsx',
                      sheet_name='Sheet1',
                      index=False,
                      float_format="%.2f")
...
with sync_playwright() as playwright:
    ...
    # df_all = pd.read_excel(r'.\htmls_数据_all_dq.xlsx', parse_dates=['game_date'], date_format='%Y/%m/%d')
    df_all = pd.read_excel(r'.\htmls_数据_all_dq.xlsx', parse_dates=['game_date'])
    df_all['game_date'] = pd.to_datetime(df_all['game_date'], format='%Y/%m/%d').dt.date
    df = df_all.loc[:]
    ...
    for (
    qihao, url_order, game_date, shaishi, zhudui_name, kedui_name, zhudui_paiming, kedui_paiming, zhudui_scored_all,
    kedui_scored_all, ouzhi_urls, yazhi_urls, shuju_urls, touzhu_urls) in zip(
            df.qihao, df.url_order, df.game_date, df.shaishi, df.zhudui_name, df.kedui_name,
            df.zhudui_paiming, df.kedui_paiming, df.zhudui_scored_all, df.kedui_scored_all,
            df.ouzhi_urls, df.yazhi_urls, df.shuju_urls, df.touzhu_urls):

        print(f"Processing qihao={qihao}")

        # Fetch and process data
        data_欧 = run_ou(playwright, qihao, url_order, game_date, zhudui_name, kedui_name, zhudui_paiming,
                         kedui_paiming, zhudui_scored_all, kedui_scored_all, ouzhi_urls)
        data_亚 = run_ya(playwright, yazhi_urls, qihao)
        data_析1, data_析2 = run_fenxi(playwright, shuju_urls, qihao, url_order, game_date, zhudui_name, kedui_name,
                                       zhudui_paiming, kedui_paiming, zhudui_scored_all, kedui_scored_all)
        data_必发 = run_bf(playwright, touzhu_urls, qihao, url_order, shaishi, game_date)
        ...
        # Increment counter and check if we need to save data
        counter += 1
        if counter % 70 == 0:
            save_data(data_欧_all, data_亚_all, data_析1_all, data_析2_all, data_必发_all, file_suffix)
            file_suffix += 1
            # Clear dataframes to save memory
            data_欧_all = pd.DataFrame()
            data_亚_all = pd.DataFrame()
            data_析1_all = pd.DataFrame()
            data_析2_all = pd.DataFrame()
